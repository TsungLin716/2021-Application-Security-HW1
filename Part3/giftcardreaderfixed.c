    ...
    while (1) {
        ...
        switch (*pc) {
            ...
            case 0x08:
                goto done;
            case 0x09:
                pc += (unsigned char)arg1;// fixed for hang.gft, changed the type from char to unsigned char
                break;
            case 0x10:
                if (zf) pc += (unsigned char)arg1;//fixed for fuzzer1.gft, changed the type from char to unsigned char
                break;
        }
        pc+=3;
        if (pc > program+256) 
...
struct this_gift_card *gift_card_reader(FILE *input_fd) {
	...
	while (!feof(input_fd)) {
        ...
        {
            return NULL;             //fix for crash1, return NULL to not pass num_bytes
        }
...
int main(int argc, char **argv) {
	...
	if (argv[1][0] == '1') 
    {
        if(thisone != NULL)
        {
             print_gift_card_info(thisone);// Fixed for crash1, make sure it is not NULL(num_bytes >= 0)
        }
    ...
    else if (argv[1][0] == '2') 
    {
        if(thisone != NULL)
        {
             gift_card_json(thisone); // Fixed for crash1, make sure it is not NULL(num_bytes >= 0)
        }
